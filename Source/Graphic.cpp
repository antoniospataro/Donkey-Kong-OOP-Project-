...
void Graphic::drawMario(Mario& m,DonkeyKong& dk){
        ...
        else if(m.getLeft()&&m.getFall()==false&&!m.getScale()/*togliere se si vuole muovere nelle scale*/)
        {
                m.setReverse(false);
                ...
                if((((matrix[m.getY()/16][m.getX()/16]==2 || matrix[m.getY()/16][m.getX()/16]==1) && m.getX()%16==0) || (m.getY()%16!=0&&matrix[m.getY()/16+1][m.getX()/16]==1)  /*|| (matrix[(m.getY()+8)/16][m.getX()/16]==2 || matrix[(m.getY()+8)/16][m.getX()/16]==1))||(m.getX()%16!=0&&((matrix[m.getY()/16][m.getX()/16+1]==2 || matrix[m.getY()/16][m.getX()/16+1]==1)   || (matrix[(m.getY()+8)/16][m.getX()/16+1]==2 || matrix[(m.getY()+8)/16][m.getX()/16+1]==1))*/)){
                        m.setScale(true);
                        if(m.getScale()){
                        m.setY(m.getY()-8);
                        m.setCont(m.getCont()+1);
